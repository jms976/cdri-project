...
interface SearchProps {
  initValue: string;
  initTarget: string;
  setInitValue: (state: string) => void;
  setInitTarget: (state: string) => void;
}
const Search: React.FC<SearchProps> = ({ ...props }) => {
  ...
  const [searchValue, setSearchValue] = useState(props.initValue || "");
  const [currentText, setCurrentText] = useState(props.initValue || "");
  ...
  const [currentTarget, setCurrentTarget] = useState(
    props.initTarget || "title"
  );
  const [searchTarget, setSearchTarget] = useState(props.initTarget || "title");

  const { data, isError, isLoading, isPreviousData } =
    useQuery<SearchBookDataTypes>(
  ...
  const handleOnKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    props.setInitValue(currentText);
  };
    ...
    <S.Container>
                ...
                <S.DetailSearchPopup>
                  ...
                  <div className="searchWrapper" id="target">
                    ...
                    >
                      {
                        targetList.find(
                          (target) => target.value === currentTarget
                        )?.label
                      }
                      <AiOutlineDown />
                    </div>
                  ...
                  <Button
                    onClick={() => {
                      ...
                      props.setInitTarget(currentTarget);
      ...
      <S.ResultSearch>
        <span className="label">
          {searchValue && `"${searchValue}" `}검색결과(
          {targetList.find((target) => target.value === searchTarget)?.label})
        </span>
        <span className="count">
          총 <span>{data?.meta.pageable_count || 0}</span>건
        </span>
      </S.ResultSearch>
